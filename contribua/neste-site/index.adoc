= Contribuindo na construção deste site
:page-layout: base
:toc:
:toc-title!:
:toc-placement!:
...
=== Configurando o teu ambiente

==== Utilizando o Fedora
Se você estive utilizando o Fedora, siga os passos na página link:utilizando-o-fedora/["Utilizando o Fedora para construir este site"].
...
Em alguns sistemas, como no {uri-fedora}[Fedora], o docker talvez precise ser executado como `root`. Então, se você tiver experimentado um erro na execução do comando acima, tente executá-lo novamente com o seguinte comando: `sudo !!`.
...
=== Editando o site e testando-o localmente
...
== Utilizando o Guard e o LiveReload

* LiveReload:
** http://aslakknutsen.github.io/blog/2012/06/16/2940571/
** http://asciidoctor.org/docs/editing-asciidoc-with-live-preview/
* Bug na execução do Guard:
** https://github.com/celluloid/celluloid/issues/619
** http://garfieldnate.github.io/blog/2015/07/20/hacking-through-an-octopress-update/

== Publicando o site

A publicação deste site é realizada automaticamente, após cada `merge` de um `pull request` no {uri-sislegis-site}[repositório GitHub do projeto], pelo uso da ferramenta de integração contínua {uri-travis-ci}[Travis CI]. Quando o site é publicado através dessa ferramenta o estado de sua construção é apresentado na página {uri-sislegis-site-travis-builds}.
